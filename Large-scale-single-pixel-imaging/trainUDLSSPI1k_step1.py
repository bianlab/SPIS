...
from net.UDLSSPI1k_step1_train import *
from utils import *
import cv2
import random
...
def sample_images(batches_done,model,X_test,y_test): 
    ...
    # 重建图像保存路径
    save_image(img_sample, "%s/%s.png" % ('results/results1k/step1/recon', batches_done), nrow=5, normalize=True)

    # 不确定性图保存路径
    save_image(var, "%s/%s.png" % ('results/results1k/step1/var', batches_done), nrow=5, normalize=True)
...
def main():
    ...
    for epoch in range(n_epochs):
        ...
        if psnr_train>psnr:
            psnr = psnr_train
            torch.save(model.state_dict(), "weights/step1/SPIS_%d_psnr%d.pth" % (epoch,psnr_train)) #要改
