...
# 预训练权重
#pre_weights_path = './weights/UDLSSPI_STEP2.pth'
...
def sample_images(batches_done,model,X_test,y_test): 
    ...
    # 重建图像保存路径
    save_image(img_sample, "%s/%s.png" % ('results/results1k/step2/recon', batches_done), nrow=5, normalize=True)

    # 不确定性图保存路径
    save_image(var, "%s/%s.png" % ('results/results1k/step2/var', batches_done), nrow=5, normalize=True)
...
def main():
    ...
    for epoch in range(n_epochs):
        for i, data in enumerate(loader, 0):
            ...
            hr_ = torch.mul(y, s)
            loss = criterion(sr_, hr_) + 2 * torch.mean(sr[1])
        ...
        if psnr_train>psnr:
            psnr = psnr_train
            torch.save(model.state_dict(), "weights/step2/SPIS_%d_psnr%d.pth" % (epoch,psnr_train)) #要改
